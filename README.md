...
| monitor | Optional | true | Set to false for a write-only register. The register is never polled, so it suits devices that map writes and reads to separate addresses. Every value published to its `set_topic` is written, including repeats, because there is no polled value to compare against. |
