...
| read_blocks | Optional | N/A | A list of `{start, count}` address ranges the device can read in one request, such as `[{start: 256, count: 104}]`. The monitored registers inside a block are read together, from the first to the last, including the unmonitored addresses between them. A sparse register map then costs one read per block. Registers outside every block batch as usual. Keep each block inside a range the device accepts, and within the 125-register Modbus read limit. |
...
| unit | Optional | device_address | The modbus unit to read and write this register on. Registers on every unit share one connection, which suits gateways and controllers that expose several units. |
